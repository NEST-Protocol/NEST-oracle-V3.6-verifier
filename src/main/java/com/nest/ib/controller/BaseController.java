...
public class BaseController {
    ...
    public ModelAndView miningData() {
        ...
        mav.addObject("nToken", erc20State.nToken);
        ...
        mav.addObject("ready", hedgeService.isReady());
    ...
    @PostMapping("/updateNtokenHedge")
    public R updateNtokenHedge() {
        erc20State.nToken.setHedge(!erc20State.nToken.isHedge());
        return R.ok();
    }

    @PostMapping("/updateTokenHedge")
    public R updateTokenHedge() {
        erc20State.token.setHedge(!erc20State.token.isHedge());
        return R.ok();
    }

    @PostMapping("/updateHedgeApi")
    public R updateHedgeApi(@RequestParam(name = "apiKey") String apiKey,
                            @RequestParam(name = "apiSecret") String apiSecret) {
        boolean ok;
        try {
            ok = hedgeService.updateApi(apiKey, apiSecret);
        } catch (Exception e) {
    ...
    public R addToken(@RequestParam(name = "tokenAddress") String tokenAddress,
                      @RequestParam(name = "tokenExchangeType") String tokenExchangeType,
                      @RequestParam(name = "tokenTradingPair") String tokenTradingPair,
                      @RequestParam(name = "nTokenExchangeType", required = false) String nTokenExchangeType,
                      @RequestParam(name = "nTokenTradingPair", required = false) String nTokenTradingPair,
                      @RequestParam(name = "fixedPrice", required = false) BigDecimal fixedPrice) {
        R r = null;
        try {
            boolean b = erc20State.updateErc20State(tokenAddress, tokenExchangeType, nTokenExchangeType, tokenTradingPair, nTokenTradingPair, fixedPrice);
            if (b) {

...
public class EthClient {
    ...
    /**
     * Check if the Ntoken issue has reached 5 million
     *
     * @return
     */
    public boolean needOpenPost2() {
        ...
        if (nTokentotalSupply == null) {
            log.error("NTokenTotalSupply retrieval failed");
            return false;
        }
        if (nTokentotalSupply.compareTo(nestProperties.getnTokenMaxTotalSupply()) > 0) {
            log.info("{} If the issuance of tokens is greater than 5 million, Post2 offers are open and NToken orders can be made", erc20State.nToken.getSymbol());
            return true;
        }
        return false;
    }
    ...
    public void approveToNestMinningContract(Wallet wallet) throws Exception {
        ...
        String nTokenApproveHash = erc20Appprove(wallet, erc20State.nToken, nonce);
    ...
    public String erc20Appprove(Wallet wallet, Erc20State.Item token, BigInteger nonce) throws ExecutionException, InterruptedException {
        ...
        if (approveValue.compareTo(new BigInteger("100000000000000")) <= 0) {
            List<Type> typeList = Arrays.<Type>asList(
                    new Address(nestProperties.getCurrentMiningAddress()),
    ...
    public boolean withdraw(String tokenAddress, BigInteger tokenAmount, Wallet wallet) {
        ...
        try {
            RawTransaction rawTransaction = RawTransaction.createTransaction(
                    ...
                    nestProperties.getCurrentMiningAddress(),
                    BigInteger.ZERO,
                    encode);
    ...
    public BigInteger allowance(Wallet wallet, String erc20TokenAddress) {
        ...
        try {
            approveValue = ContractBuilder.erc20Readonly(erc20TokenAddress, Web3jHelper.getWeb3j()).allowance(wallet.getCredentials().getAddress(), nestProperties.getCurrentMiningAddress()).sendAsync().get();
        } catch (InterruptedException e) {
     ...
     * @return
     */
    public BigInteger lastIndex(String tokenAddress) {
        List<PriceSheetView> list = list(tokenAddress, BigInteger.ZERO, BigInteger.ONE, BigInteger.ZERO);
     ...
     * @return
     */
    public List<PriceSheetView> unClosedSheetListOf(String miner, String tokenAddress, BigInteger maxFindNum) {
        List<PriceSheetView> list = list(tokenAddress, BigInteger.ZERO, maxFindNum, BigInteger.ZERO);
    ...
    public List<PriceSheetView> list(String tokenAddress, BigInteger offset, BigInteger count, BigInteger order) {
        List<PriceSheetView> sheetList = null;
        try {
            sheetList = ContractBuilder.nestMiningContract(Web3jHelper.getWeb3j()).list(tokenAddress, offset, count, order).send();
        } catch (Exception e) {
     ...
     * @return
     */
    public List<Uint256> canClosedSheetIndexs(String miner, String tokenAddress, BigInteger maxFindNum) {
        List<PriceSheetView> list = unClosedSheetListOf(miner, tokenAddress, maxFindNum);
     ...
     * @return
     */
    public List<PriceSheetView> unVerifiedSheetList(String address) {
        List<PriceSheetView> list = list(address, BigInteger.ZERO, verifyState.getMaxFindNum(), BigInteger.ZERO);
        if (CollectionUtils.isEmpty(list)) return null;
     ...
     * @param indices
     */
    public String closeList(Wallet wallet, BigInteger nonce, String address, List<Uint256> indices) {
